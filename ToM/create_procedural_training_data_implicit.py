...
This version uses ONLY implicit belief conditions (0_ prefix).

This script:
1. Loads forward_belief examples from 0_ prefix conditions (implicit beliefs)
2. Samples 1,400 examples total (700 false belief + 700 true belief)
...
def sample_training_data(
    conditions_dir: Path,
    num_per_condition: int = 700,
    random_seed: int = 42
) -> Tuple[List[Dict], Dict]:
    """
    Sample training examples from 0_ prefix forward_belief conditions ONLY.

    Args:
        conditions_dir: Path to procedural-evals-tom/data/conditions/
        num_per_condition: Number of examples to sample per condition (default 700)
    ...
    # Define the 2 conditions to sample from (ONLY 0_ prefix - implicit beliefs)
    conditions = [
    ...
    training_pairs = []
    metadata = {
        'condition_type': 'implicit_beliefs',
        'prefix': '0',
    ...
    for condition in conditions:
        csv_path = conditions_dir / condition / 'stories_original.csv'

        # Fall back to stories.csv if original doesn't exist
...
def main():
    ...
    print("=" * 80)
    print("Creating ToM Training Dataset - IMPLICIT BELIEFS (0_ prefix)")
    print("=" * 80)
    print(f"\nConditions directory: {conditions_dir}")
    print(f"Output directory: {output_dir}\n")

    # Sample training data
    print("Sampling training examples from 0_ prefix conditions...")
    print("  - 700 examples from 0_forward_belief_false_belief")
    print("  - 700 examples from 0_forward_belief_true_belief")
    print()

    training_pairs, metadata = sample_training_data(
        conditions_dir=conditions_dir,
        num_per_condition=700,
        random_seed=42
    )
    ...
    # Save training data
    training_data_path = output_dir / 'procedural_training_data_implicit.json'
    ...
    # Save metadata
    metadata_path = output_dir / 'procedural_training_metadata_implicit.json'
